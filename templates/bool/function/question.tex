
{{tr("Study the following function:")}}

$F(A,B,C,D) ={{  sop_quest|normalize_formula }}$

{% if RENDER_MODE == "question" %}

 {% endif %}

{% if RENDER_MODE == "answer" %}

${{ function_name }} = {{ minterms }}$

\textbf{ {{tr("Don't Care")}} }

${{ function_name }} = {{ dontcares }}$



\textbf{ {{tr("Truth Table")}} }

{{ truthtable }}


\begin{tabular}{|c|c|c|c||c|}
\hline
{{ variables[0] }} & {{ variables[1] }} & {{ variables[2] }} & {{ variables[3] }} & F \\
{% for i in range(16) %}
    {% if i%4==0 %}
        \hline
    {% endif %}
  {% set bits = '{:04b}'.format(i) %}
  {% set bit_list = bits | list %}
  {{ bit_list[0] }} & {{ bit_list[1] }} & {{ bit_list[2] }} & {{ bit_list[3] }} & {{ '1' if i in minterms else 'x' if i in dontcares else 0 }} \\
{% endfor %}
\hline
\end{tabular}


\textbf{ {{tr("Canonical Forms")}} }
\begin{itemize}
\item ${{ function_name }}(A,B,C,D) =  {{  dnf_dict.default|normalize_formula }}$
\item ${{ function_name }}(A,B,C,D) = {{  cnf_dict.default|normalize_formula }}$
 \item ${{ function_name }}(A,B,C,D) =  \sum({{  minterms| join(", ") }})$
 \item ${{ function_name }}(A,B,C,D) =  \prod({{  maxterms| join(", ") }})$
\end{itemize}




 {% if method == "nand" %}
 \textbf{ NAND forms  \aRL{بوابات نفي الوصل}}
{{ nand_sop }}

{{ nand_dnf }}

 {% endif %}

{% if method == "nor" %}
 \textbf{ NOR forms  \aRL{بوابات نفي الفصل}}
{{ nor_pos }}

{{ nor_cnf }}

 {% endif %}




\textbf{Karnaugh Table \aRL{جدول كارنوف}}

\begin{karnaugh-map}[4][4][1][{{ cd }}][{{ ab }}]
  \minterms{ {{ minterms | join(", ") }} }
  \maxterms{ {{ maxterms | join(", ") }} }
  {% if dontcares %}
    \terms{ {{ dontcares | join(", ") }} }{X}
  {% endif %}

       {{ simplification |format_map_terms(method=method.lower())|join("\n") }}

 \end{karnaugh-map}

     {{tr("Simplified Sum of products")}}  : ${{ sop_dict.default|normalize_formula }}$\\
     {{tr("Simplified product of sums")}}  : ${{ pos_dict.default|normalize_formula }}$\\

\textbf{ {{tr("Logic diagram")}} }

{{ logicdiagram }}


{% include "bool/logigram/structured_logigram.tex.j2" %}

{% endif %}
