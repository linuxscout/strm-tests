...
{% set synch_type = data.synch_type if data.synch_type in ("dual", "falling", "rising", "no") else "no" %}
{%set clock_signal ="$"~data.clock.name~"$ & [timing/c/"~synch_type~" arrows] C"~data.length~"{"~data.clock.period~"C}"%}
{{clock_signal}}
{%endmacro %}
{#----------------------------------------------------------
...
----------------------------------------------------------#}
{%- macro draw_out_signals(output_signals, mode="question") -%}
{% if mode == "question" %}
...
\begin{scope}[blue,semitransparent,densely dotted,thin]
\vertlines{2,4,...,\twidth}
\end{scope}
{% endif %}
...
{{ draw_out_signals(data.output_signals,mode=mode)}}\\
\extracode
