...
%%  \hfill\aRL{أنجز التحويلات الآتية}
...
\begin{enumerate}
  \item {{tr("Convert from base {in_base} to base 10", in_base=in_base)}}
  \item {{tr("Convert from base 10 to base {out_base}", out_base=out_base)}}
\end{enumerate}
{% endif %}
...
{% set digits = steps_to10 %}
  \textbf{ {{tr("Convert from base {in_base} to base 10", in_base=in_base)}} }
...
{% if steps_from10 %}
\textbf{ {{tr("Convert from base 10 to base {out_base}", out_base=out_base)}} }\\
 {% set number_b10 = number_tmp if number_tmp else number %}
{{ division_process(number_b10, out_base, steps_from10) }}
{% endif %}

{{tr("Result")}} (bottom→top remainders): {{ output }}
