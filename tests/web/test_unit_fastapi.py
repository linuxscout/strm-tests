...
def test_submit_quiz(client):
    ...
    response = client.post(
        "/submit", json={"command": "random", "category": "random", "args": {}}
    )
    assert response.status_code == 200
...
def test_get_commands(client):
    ...
    assert isinstance(
        response.json(), dict
    ), f"The result type is {type(response.json())}"  # Check if the response is a dictionary

    # Test with a category filter
    response = client.get(
        "/api/commands?category=some_category"
    )  # Replace 'some_category' with an actual category
    assert response.status_code == 200
    assert isinstance(response.json(), dict)  # Check if the response is a dictionary
...
def test_get_random_commands(client):
    ...
    assert response.status_code == 200
    assert (
        "commands_list" in response.json()
    )  # Check if the response contains 'commands_list'
...
def test_submit_valid_specific_command(client):
    ...
    args = {"float": {"float": "15"}}
    response = client.post(
        "/submit", json={"command": "float", "category": "encoding", "args": args}
    )
    assert response.status_code == 200
    assert "question" in response.text
    assert "answer" in response.text
...
def test_submit_invalid_category(client):
    ...
    response = client.post(
        "/submit", json={"command": "float", "category": "not_a_category", "args": {}}
    )
...
def test_submit_invalid_command(client):
    ...
    response = client.post(
        "/submit", json={"command": "not_a_command", "category": "encoding", "args": {}}
    )
    assert response.status_code == 400
...
def test_submit_command_not_in_category(client):
    ...
    response = client.post(
        "/submit", json={"command": "float", "category": "boolean algebra", "args": {}}
    )
    assert response.status_code == 400
...
def test_all_commands_via_api_simple(client: TestClient, builder):
    ...
    for cmd in commands:
        ...
        ), f"/submit failed for command '{cmd}', status code {response.status_code} details:{response.text}"
        assert (
            "question" in response.text.lower()
        ), f"No question rendered for command '{cmd}', status code {response.status_code} details:{response.text}"
        assert (
            "answer" in response.text.lower()
        ), f"No answer rendered for command '{cmd}', status code {response.status_code} details:{response.text}"
...
def test_all_commands_via_api(client: TestClient, builder):
    ...
    for cmd in commands:
        ...
        if response.status_code != 200:
            failed.append(
                (cmd, f"status={response.status_code}, details={response.text}")
            )
            continue
    ...
    for cat, meta in categories.items():
        ...
        for cmd in commands:
            ...
            if response.status_code != 200:
                ...
                pytest.fail(
                    f"/submit failed for category '{cat}', command '{cmd}', "
                    f"status={response.status_code}, details={details}"
                )

            assert (
                "question" in response.text.lower()
            ), f"No question rendered for category '{cat}', command '{cmd}', status code {response.status_code} details:{response.text}"
            assert (
                "answer" in response.text.lower()
            ), f"No answer rendered for category '{cat}', command '{cmd}', status code {response.status_code} details:{response.text}"
