...
class TestFormatter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # إنشاء قالب للسؤال
        with open(
            os.path.join(template_base, "question.tex"), "w", encoding="utf-8"
        ) as f:
            text = (
                'سؤال: {{ arabic_text }}"nQuestion: {{ question_text }}\nNumber: {{ number }}'
        ...
        with open(
            os.path.join(trans_base, "translations.json"), "w", encoding="utf-8"
        ) as f:
            text = "{}"
            f.write(text)
        # إعداد Formatter
        self.formatter = quiz_format(
            lang="ar-en", formatting="tex", templates_dir=self.temp_dir.name
        )
