...
def test_question_static_nand_exp(builder):
    ...
    assert "minterms" in result, f"the result is {result}"
    assert (
        result["sop"] == " a'.b'.c + a'.b'.d "
    ), f"the result['sop'] is {result['sop']}"
    assert result["pos"] == "(a').(b').(c+d)", f"the result['pos'] is {result['pos']}"


def test_question_multi_funct(builder):
    result = builder.question_multi_funct(
        minterms_list=[[1, 2, 3]], dont_care_list=[[]]
    )
    assert isinstance(result, dict)
    assert "data_list" in result, f"the result is {result}"
    assert "minterms" in result["data_list"][0], f"the result is {result['data_list']}"
    assert (
        result["data_list"][0]["sop"] == " a'.b'.c + a'.b'.d "
    ), f"the result['sop'] is {result['data_list'][0]['sop']}"
    assert (
        result["data_list"][0]["pos"] == "(a').(b').(c+d)"
    ), f"the result['pos'] is {result['data_list'][0]['pos']}"
...
def test_question_exp(builder):
    ...
    assert "minterms" in result, f"the result is {result}"
    assert (
        result["sop"] == " a.b.c.d + a'.b'.d + a.b.c'.d' "
    ), f"the result['sop'] is '{result['sop']}'"
    assert (
        result["pos"] == "(a+d).(a+b').(a'+b).(c'+d).(b'+c+d')"
    ), f"the result['pos'] is '{result['pos']}'"
    ...
    assert "minterms" in result, f"the result is {result}"
    assert (
        result["sop"] == " a'.b' + a.b.c.d "
    ), f"the result['sop'] is '{result['sop']}'"
    assert (
        result["pos"] == "(a+b').(a'+b).(b'+c).(b'+d)"
    ), f"the result['pos'] is '{result['pos']}'"
    assert (
        result["sop_quest"] == sop_quest
    ), f"the result['sop_quest'] is '{result['sop_quest']}'"


def test_prepare_logigram_list(builder):
    sop_list = ["A.B + C.D"]
    equations_list = [{"sop": "A.B + C.D"}]
    result = builder._prepare_logigram_list(
        sop_list,
        function_namelist=["F"],
        variables=["A", "B", "C", "D"],
...
def test_invalid_minterms(builder):
    with pytest.raises(TypeError):
        builder.question_multi_funct(
            minterms_list=[[1, 2, 3, "X", 20]], dont_care_list=[[]]
        )
