...
class TestFloatPoint:
    ...
    def test_decimal_converter(self, float_point_instance):
        ...
        for x, y in test_set:
            ...
            assert  res == y, f"Error in testing decimal converter {x} => {res} must be {y}"


    def test_float_bin(self, float_point_instance):
        # Test float_bin for various float inputs
        test_set = [(5.125, "101.001"),
                    (-3.75, "-0b11.110"),
                    (0.5, '.100'),
                    ]
        for x, y in test_set:
            res = float_point_instance.float_bin(x, places=3)
            assert  res == y, f"Error in testing float_bin converter {x} => {res} must be {y}"
    ...
    def test_ieee754_components(self, float_point_instance):
        ...
        assert result['mantissa'] == '00000111010011001100110'  # Adjust based on expected mantissa
        assert result['hex'] == '4383A666'  # Adjust based on expected hex representation
        ...
        assert result['exponent'] == 8  # Adjust based on expected exponent
        assert result['mantissa'] == '00000111010011001100110'  # Adjust based on expected mantissa
        assert result['hex'] == 'C383A666'  # Adjust based on expected hex representation
