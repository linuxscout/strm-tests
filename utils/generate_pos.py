...
def make_key(key):
    """
    convert key into new keys
    """
    old_key = key
    # save variables inversed
    for var in ('a', 'b','c','d'):
        notvar = var+"'"
        key = key.replace(notvar, notvar.upper() )
    # inverse positive vars
    for var in ('a', 'b','c','d'):
        notvar = var+"'"
        key = key.replace(var, notvar )
        
    # inverse negative vars
    for var in ('a', 'b','c','d'):
        notvar = var.upper()+"'"
        key = key.replace(notvar, var )
    key = key.replace(".","+")
    # ~ key = "("+key+")"
    return key
    # ~ print(old_key, key)
...
def main(args):
    ...
    for key in TEX_REDUCTION_TABLE:
        new_key = make_key(key)
        TEX_REDUCTION_TABLE_POS[new_key]=   TEX_REDUCTION_TABLE[key]   
    pprint.pprint(TEX_REDUCTION_TABLE_POS)
    print("TEX_REDUCTION_TABLE_POS={")
    for k in sorted(TEX_REDUCTION_TABLE_POS, key=len):
        print("\"%s\":\"%s\","%(k, TEX_REDUCTION_TABLE_POS[k]))  
    print("}")         
