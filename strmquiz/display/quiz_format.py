...
class quiz_format:
    ...
    def translate(
        self, key: str, mode: str = "inline", lang: str = "", **kwargs
    ) -> str:
    ...
    def group_digits_by_4(self, value):
        try:
            ...
            int_groups = [
                int_part[max(i - 4, 0) : i] for i in range(len(int_part), 0, -4)
            ]
            ...
            if frac_part:
                frac_groups = [
                    frac_part[i : i + 4] for i in range(0, len(frac_part), 4)
                ]
    ...
    def render_question_answer(
        self, template_base: str, context: dict
    ) -> tuple[str, str]:
        """
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Template '{e.name}' not found in {self.templates_dir}."
            )
