...
class SequentialQuestionBuilder(Question_Builder):
        ...
        "flip": {
            ...
            "args": {
                ...
                "length": {
                    ...
                    "default": 10,
                    "range": [1, 100],
                    "label": "Length of timing diagram",
                },
                "synch_type": {
                    "type": "string",
                    "default": "rising",
        ...
        "counter": {
            ...
            "args": {
                ...
                "counter_flips": {
                    "type": "list",
                    "default": ["JK", "JK"],
                    "label": "Counter flip flop type list",
                },
                "counter_nbits": {
                    "type": "integer",
                    "default": 2,
        ...
        "register": {
            ...
            "args": {
                ...
                "register_flips": {
                    "type": "list",
                    "default": ["D", "D"],
                    "label": "Register flip flop type list",
                },
                "register_nbits": {
                    "type": "integer",
                    "default": 2,
                    "range": [1, 16],
                    "label": "Register bits number",
                },
                "register_random": {
                    "type": "boolean",
                    "default": False,
                    "label": "Random register",
                },
            },
        },
        "seq_misc": {
    ...
    def _preprare_chrnonogram(
        ...
        out_signals_initial = {
            k: v for k, v in init_signals.items() if k in output_vars
        }
    ...
    def _preprare_chrnonogram_register(
        self,
        input_vars=[
            "V",
        ],
        ...
        reg_sim = registersimulator.RegisterSimulator(
            inputs=input_vars,
            outputs=output_vars,
        ...
        tmp_signals = reg_sim.resolve_register(
            init_signals.copy(), signal_length=length
        )
    ...
    def _preprare_chrnonogram_counter(
        self,
        input_vars=[
            "V",
        ],
        ...
        # start_signals = start_signals*
        flip_types = [x.get("type", "") for x in flip_list]
        reg_sim = countersimulator.CounterSimulator(
            inputs=input_vars,
            outputs=output_vars,
            flip_types=flip_types,
        ...
        for key in start_signals:
            if key in output_vars:
                ...
                if key == "Vcc":
                    ...
                    signal_dict = chrono.question(
                        {key: start_signals[key]}, length=length
                    )
                    init_signals[key] = signal_dict[key]
        # set synchronization type
        chrono.set_synch_type(synch_type)
    ...
    def question_chronogram(
        ...
        data = self._preprare_chrnonogram(
            input_vars=input_vars,
            start_signals=varlist,
            flip_type=flip_type,
            length=length,
            synch_type=synch_type,
            output_vars=output_vars,
    ...
    def question_flip(
        ...
        data = self._preprare_chrnonogram(
            input_vars=flip_data["inputs"],
            start_signals=start_signals,
            flip_type=flip_type_new,
            length=length,
            synch_type=synch_type,
            output_vars=output_vars_new,
        )
    ...
    def question_register(
        ...
        if register_random:
            ...
            reg_flip_list = [
                seqconst.FLIPS_DATA.get(ft, {}) for ft in reg_flip_type_list
            ]
        else:
            reg_flip_type_list = flip_types
            # in case that configuration fliptypes is missing
            if len(reg_flip_type_list) < nbits:
                reg_flip_type_list = reg_flip_type_list + ["D"] * (
                    nbits - len(flip_types)
                )
            # get data from flip standard
            reg_flip_list = [
                seqconst.FLIPS_DATA.get(ft, {}) for ft in reg_flip_type_list
        ...
        data = self._preprare_chrnonogram_register(
            input_vars=inputs,
    ...
    def question_counter(
        ...
        if counter_random:
            ...
            if len(reg_flip_type_list) < nbits:
                ...
                reg_flip_type_list = reg_flip_type_list + ["JK"] * (
                    nbits - len(flip_types)
                )
            # get data from flip standard
            reg_flip_list = [
        ...
        data = self._preprare_chrnonogram_counter(
            input_vars=inputs,
            start_signals=init_signals,
            flip_list=reg_flip_list,
            length=length,
