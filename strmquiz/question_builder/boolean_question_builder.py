...
class BooleanQuestionBuilder(Question_Builder):
    ...
    def command_static_funct(self, args={}):
        ...
        return self.question_static_funct(
            minterms=minterms,
    ...
    def command_nand_funct(self, args={}):
        minterms = args["minterms"][0] if args.get("minterms", []) else []
        dont_care = args["dontcare"][0] if args.get("dontcare", []) else []
        return self.question_static_nand_exp(
            minterms=minterms,
            var_names=args.get("var_names", []),
            output_names=args.get("output_names", []),
            dont_care=dont_care,
    ...
    def command_map_for_sop(self, args={}):
        return self.question_map_for_sop(
            nb=args.get("functions_number", 2), minterms_list=args.get("minterms", [[]])
        )

    def command_exp(self, args={}):
        minterms = args["minterms"][0] if args.get("minterms", []) else []
        sop_quest = args.get("sop_question", "")
        return self.question_exp(minterms=minterms, sop_quest=sop_quest)

    def command_multi_funct(self, args={}):
        return self.question_multi_funct(
            minterms_list=args.get("minterms", [[]]),
            var_names=args.get("var_names", []),
            output_names=args.get("output_names", []),
            dont_care_list=args.get("dontcare", [[]]),
    ...
    def question_funct(
        ...
        context = self._prepare_kmap_data(
            minterms=minterms, dontcares=[], correct=True, variables=self.bq.variables
        )
        ...
        logigramdict = self._prepare_logigram_list(
            [
                sop,
            ],
            function_namelist=[
    ...
    def question_static_funct(
        self,
        minterms,
        var_names=["A", "B", "C", "D"],
        output_names=["S0", "S1", "S2", "S3"],
        dont_care=[],
    ):
        ...
        context["logicdiagram"] = ""
        context["terms"] = [
            [t.strip() for t in term.split(".")] for term in sop.split("+")
        ]
        equations_list = [
            {
                "sop": sop,
    ...
    def question_multi_funct(
        ...
        dont_care_list = self.bq.validate_terms_list(
            dont_care_list, name="dontcare_list"
        )
        for minterms, dont_care, fname in zip(
            minterms_list, dont_care_list, output_names
        ):
            data = self._prepare_kmap_data(
                minterms=minterms,
                dontcares=dont_care,
                correct=True,
                variables=var_names,
        ...
        logigramdict = self._prepare_logigram_list(
            sop_list,
            function_namelist=output_names,
            variables=var_names,
            method=method,
            equations_list=equations_list,
        )
    ...
    def _prepare_logigram_list(
        ...
        lgdict = lg.prepare_logigram_list(
            sop_list, function_namelist, equations_list=equations_list
        )
        ## format labels
