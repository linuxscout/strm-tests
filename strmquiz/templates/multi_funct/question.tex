...
\item \textbf{First Canonical Forms \aRL{ الأشكال القانونية الأولى}}
    \begin{itemize}
    {% for data in data_list %}
        \item ${{ data.function_name }}(A,B,C,D) =  {{  data.dnf_dict.formatted }}$
    {% endfor %}
    \end{itemize}
...
\item \textbf{Second Canonical Forms \aRL{ الأشكال القانونية الثانية}}
    \begin{itemize}
    {% for data in data_list %}
        \item ${{ data.function_name }}(A,B,C,D) =  {{  data.cnf_dict.formatted }}$
    {% endfor %}
    \end{itemize}
...
\end{itemize}
 ...
 {% for data in data_list %}
    ...
    \item ${{ data.function_name }}$ = ${{ data.nand_sop_dict.formatted }}$

    Explanation
    \begin{itemize}
        \item ${{ data.function_name }}$ = ${{ data.sop_dict.formatted }}$
...
{% if method.lower() == "nor" %}
 ...
 {% for data in data_list %}
    ...
    \item ${{ data.function_name }}$ = ${{ data.nor_pos_dict.formatted }}$

    Explanation
    \begin{itemize}
        \item ${{ data.function_name }}$ = ${{ data.pos_dict.formatted }}$
...
\begin{karnaugh-map}[4][4][1][{{ data.cd }}][{{ data.ab }}]
 ...
 \end{karnaugh-map}
{% if RENDER_MODE == "answer" %}
    Simplified Sum of products : ${{ data.function_name }} = {{ data.sop_formatted }}$\\
    Simplified product of sums : ${{ data.function_name }} = {{ data.pos_formatted }}$
{% endif %}
{% endfor %}
