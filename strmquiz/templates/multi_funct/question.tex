...
\item \textbf{First Canonical Forms \aRL{ الأشكال القانونية الأولى}}
    \begin{itemize}
    {% for data in data_list %}
        \item ${{ data.function_name }}({{variables|join(", ")}}) =  {{  data.dnf_dict.formatted }}$
    {% endfor %}
    \end{itemize}
...
\item \textbf{Second Canonical Forms \aRL{ الأشكال القانونية الثانية}}
    \begin{itemize}
    {% for data in data_list %}
        \item ${{ data.function_name }}({{variables|join(", ")}}) =  {{  data.cnf_dict.formatted }}$
    {% endfor %}
    \end{itemize}
...
\begin{karnaugh-map}[4][4][1][{{ data.cd }}][{{ data.ab }}]
 ...
 \end{karnaugh-map}
{% if RENDER_MODE == "answer" %}
    Simplified Sum of products : ${{ data.function_name }} = {{ data.sop_dict.formatted }}$\\
    Simplified product of sums : ${{ data.function_name }} = {{ data.pos_dict.formatted }}$
{% endif %}
{% endfor %}
