...
{%else%}
    {# draw a clock #}
    {{draw_clock(name="Ck", label="Ck", xpos=0, ypos=2)}}
    {# draw a modules #}
    {% set ns = namespace(name="", id="", xpos=0, prev_name="", prev_type="", type="") %}
    {% set ypos = 2 %}
    {% for i in range(size) %}
        ...
        {# draw a Vcc var #}
        {{draw_input_var(name="Vcc"~(i|string), label="Vcc", xpos=ns.xpos-1.5, ypos=ypos+2)}}
...
-------------------#}
{%macro draw_counter(flip="JKasyn", counter_type="",size=3)%}
{% set flips_list = [flip]*size %}
{% set ns = namespace(outputs = []) %}
{% for i in range(size) %}
    {% set _ = ns.outputs.append("Q" ~ i|string) %}
...
{{ define_basic_flip(type="jkasyn", id="JKAMod", output_var="Q")}}
...
\begin{circuitikz}
{{place_flip(id="JKAMod", name="jk4", xpos=5, ypos=4)}}
\end{circuitikz}