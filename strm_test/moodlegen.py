...
class MQG_builder:
    ...
    def convertbase(self, base1=0, base2=0, start=0 ,end = 0, randomize=False):
        ...
        for i in range(start,end):
            ...
            if randomize:
                base_list = [(2,16),(16,2),(10,16), (16,10), (8,16), (16,8), (7,5), (6,12)]
            ...
            questvalue = "::Conversion base::Convertir de la base %d à la base  %d le nombre suivant %s "%(base1, base2,nb_source)            
            answers =  [u"%s"%nb_dist]
            wrongs = []
    ...
    def comp_one_two(self, nbits = 8, start=0, end=0, randomize=False):
        ...
        for i in range(start,end):
            ...
            if i <= end/2: 
                questvalue = "::Complément à 2:: Convertir le nombre suivant -%d en complément à 2 sur  %dbits"%(n,nbits)            
                answers =  [u"%s"%comp2]
            else:
                questvalue = "::Complément à 2:: Convertir en binaire, le nombre suivant %s codé complément à 2 sur  %dbits"%(comp2,nbits)            
                answers =  [u"-%d"%n]
    ...
    def float_quest(self, n=0, iteration=10, randomize=False):
        ...
        for i in range(iteration):
            ...
            if i <= iteration/2: 
                questvalue = "::Virgule flottante:: Representer le nombre suivant en IEEE-754-32bits %f"%(n)            
                answers =  [u"%s"%hexfloat]
            else:
                questvalue = "::Virgule flottante:: Decoder le nombre suivant codé en IEEE-754-32bits %s"%(hexfloat)            
                answers =  [u"-%f"%n]
